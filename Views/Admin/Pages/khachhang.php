...
<table id="example" class="display" style="width:100%">
        ...
        <tbody>
            <?php foreach($data['khachhang'] as $item) : ?>
                <tr>
                    <td><?php echo $item->TenKH ?></td>
                    <td><?php echo $item->GioiTinh ?></td>
                    <td><?php echo date("d-m-Y", strtotime($item->NgaySinh)) ?></td>
                    <td><?php echo $item->SoDT ?></td>
                    <td><?php echo $item->Email ?></td>
                    <td><?php echo $item->TenTK ?></td>
                    <td><?php echo $item->MatKhau ?></td>
                    <td>
                        <a class="btn btn-success" href="../KhachHang/Details/<?php echo $item->MaKH ?>">Details</a>
                        <a class="btn btn-danger delete-btn" data-bs-toggle="modal" data-bs-target="#exampleModal" data-id="<?php echo $item->MaKH ?>">Delete</a>
                    </td>
                </tr>
                <?php endforeach;?>
        </tbody>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        Bạn có muốn xoá <span id="user-id"></span>
      </div>
 ...
 document.querySelectorAll(".btnModel").forEach((button,index)=>{
  button.addEventListener('click',(e)=>{
    ...
    if(id){
      $.ajax({
        type: "POST",
        url: "http://localhost/MVC_qlpt/KhachHang/Delete/",
        data: JSON.stringify({
          id:id,
        }),
        dataType: "json",
        success: function (response) {
          ...
          if(response.status === 200){
            location.reload();
                ...
                alert('Xóa thất bại! Tồn tại đơn hàng của khách hàng này');
                console.log(error);
            }
      });
    }
